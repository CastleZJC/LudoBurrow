...
| 文件 | 覆盖页面 | 对齐实现 |
|------|----------|----------|
| `LudoBurrow.html` | 主菜单、关卡选择（轨页签 + 关卡网格）、键盘游戏（四模式 + 大/小键盘双布局虚拟键盘）、拼图（五区三列）、迷宫（8 主题 + 点击/键盘移动）、暂停浮层、结算浮层、设置（语言/限时/AI/词汇表/存档备份）、方案管理（列表 + 就近编辑：新建置顶、编辑内联于对应卡片下方） | `src/components/*.vue` + `src/games/*` 同构 |
